package AsposeEmailCloud::ApiClient;
...
sub new
{
	...
	if(not defined $AsposeEmailCloud::Configuration::app_sid or $AsposeEmailCloud::Configuration::app_sid eq ''){
		croak("Aspose Cloud App SID key is missing.");
    }
	
	if(not defined $AsposeEmailCloud::Configuration::api_key or $AsposeEmailCloud::Configuration::api_key eq ''){
    ...
    my (%args) = (
        ...
        'base_url' => $AsposeEmailCloud::Configuration::api_server,
        @_
    );
...
sub call_api {
	...
	$resource_path =~ s/\Q{appSid}\E/$AsposeEmailCloud::Configuration::app_sid/g;
  
    my $_url = $self->{base_url} . $resource_path;
  
    # update signature
    my $signature = $self->sign($_url, $AsposeEmailCloud::Configuration::app_sid, $AsposeEmailCloud::Configuration::api_key);
    ...
    $self->{ua}->timeout($self->{http_timeout} || $AsposeEmailCloud::Configuration::http_timeout); 
    $self->{ua}->agent($self->{http_user_agent} || $AsposeEmailCloud::Configuration::http_user_agent);
...
sub deserialize
{
    ...
    if (not defined $data) {
        ...
        my $_instance = use_module("AsposeEmailCloud::Object::$class")->new;
        if (ref $data eq "HASH") {
